...
<%
	...
	if (objectId > 0) {
		...
		cusObject = ProGateOrgCustomerLocalServiceUtil.getProGateOrgCustomer(objectId);	
	}	
		
	if(cusObject != null) {
		objectId = cusObject.getPrimaryKey();
		cusType = cusObject.getCustomerType();
		orgName = cusObject.getCustomerName();
		logo = cusObject.getLogo();
		website = cusObject.getWebsite();
		description = cusObject.getDescription();
		count = ProGateOrgCustomerRepresenterLocalServiceUtil.countListCustRepresenterByCustId(cusObject.getPrimaryKey());
	...
	*/
	...
	if(request.getParameter(Customer_Const.TEXT_ERROR_ORG_NAME) != null){
		textErrOrgName = request.getParameter(Customer_Const.TEXT_ERROR_ORG_NAME);
	}	
%>
...
<form name="frmAdd" method="post" onsubmit="Javascript: return false;" 
	action="<portlet:actionURL><portlet:param name='struts_action' value='/customer/edit_action'></portlet:param>
	...
	<input type="hidden" name="<%= Customer_Const.OBJECT_ID %>" value="<%= objectId %>">
	...
	<div class="tq-layout">
		<div class="tq-add-header">
			<span class="tq-content-header">Sửa Khách Hàng / Đối Tác</span>
		</div>
		<div class="tq-body">
		
			<div class="tq-tr-header">
				<div style="float: right; padding-top: 11x; display:none;"><span style="float: left;padding-top: 10px" class="tq-body-content">Xem trước</span></div>
				<div class="tq-img-td" style="float: right; display: none;"><img alt="" src="/ProGate-portlet/images/preview.png" width="30px" height="25px"/></div>
			<div id="popupDel" style="display: block;">
				<div class="img-td" style="float: right;padding-top: 8px; padding-right: 5px; display: none;"><span> | </span></div>
				<div class="img-td" style="float: right;padding-right: 2px;"><img alt="" src="<%=request.getContextPath()%>/images/news_org/delete.png" width="23px" height="23px" id="popup" style="vertical-align: middle; margin-left: 3px; cursor:pointer;"/>
				<span onclick="showPopupDelete();"> Xoá </span>
				</div>
			...
			<% //if(addSuccess == 1){ %>
				<div class="tq-body-mes-success" id="mesSuccess" style="display: none;">
					...
					<div style="float: left; padding-top: 8px; padding-left: 5px;">
						<span class="tq-body-content-mes">Bạn đã sửa đối tượng thành công </span>
					</div>
			...
			<% //if(addSuccess == 2){ %>
				<div class="tq-body-mes-error" id="mesError" style="display: none;">
					...
					<div style="float: left; padding-top: 8px; padding-left: 5px;">
						<span class="tq-body-content-mes">Đã có lỗi xảy ra trong quá trình Sửa đối tượng</span>
					</div>
					<div style="float:right; padding-right:20px;">
						<img alt="" src="/ProGate-portlet/images/black_delete.png" width="28px" height="28px" onclick="closeMes();">
					</div>
			...
			<div class="tq-body">
				...
				<div class="tq-td">
					<input type="text" name="<%= Customer_Const.ORG_NAME %>" value="<%= orgName %>" size="78" id="orgName" onblur="validOrgName();" maxlength="50">
				</div>
				<div class="cus-td-img">
			...
			<div class="tq-body">
				...
				<div class="tq-td">
					<div class="cus-image" id="orgLogoDiv" style="display: block;"> 
						<img src="<%= logo %>" id="orgLogoImg" onload="scaleImg(this, 145, 100)" style="display: none;">
					</div>
				</div>
			...
			%>
				<div class="tq-body">
					<div>
						<div style="padding-top: 6px;float: left;"><span class="tq-body-content">Người Đại Diện  <%=i + 1 %></span> </div>
				...
				<div class="tq-body">
					...
					<div class="tq-td">
						<input type="text" name="<%= Customer_Const.FULL_NAME %>" value="<%= founder.getDisplayName() %>" size="55" maxlength="50">
					</div>
				</div>
				<div class="tq-body">
...
<form name="hidenDelForm" method="post" 
	action="<portlet:actionURL windowState='exclusive'>
			...
			<input type="hidden" name="<%= Customer_Const.URL %>" value="<%= url %>">
			<input type="hidden" name="<%= Customer_Const.MODE %>" value="0">
			<input type="hidden" name="<%= Customer_Const.COUNT %>" value="">
</form>
