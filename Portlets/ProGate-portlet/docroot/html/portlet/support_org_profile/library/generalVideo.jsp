...
<%
	...
	String articleType = ProGateJournalArticleTypesLocalServiceUtil.getArticleTypeOrgSupportVideo();
...
<script type="text/javascript">
	jQuery(function(){	
		jQuery(".generalArticleContent img").each(function(){
	...
	<div class="container">
		  ...
		  <c:forEach var="article" items="${listArticles}" varStatus="rowCounter">
		        <div class="folder-news">   
			          <div id="view_video" style="display: block; float: left; width: 200px; height: 200px; 
			                                      background-color: black; margin-left: 0px;margin-right:10px;">
		          	  ...
		          	  <div style="text-align: justify;"> 
						   ...
						   <c:if test="${isOrgAdmin eq true}">
						  		<img src="<%=request.getContextPath()%>/images/support_org_profile/write.png" style="vertical-align:bottom;" width="25" height="25" />
						   		<a href="${repairLibrary}&modeType=Video&articleId=${article.journalArticleId}" ><%= res.getString("repair")%></a>
						   </c:if>
						   
						   <a class="link-title-video" href="${detailURL}&actionView=detailVideo&articleId=${article.journalArticleId}">
						      ${article.title}
						   </a>
				           
						  <div style="text-align:justify; margin-left: 203px;" class="generalArticleContent">
						  	<c:choose>
		...
		<%
			...
			System.out.println("sumOfArticle =" + sumOfArticle);
