 ...
 <script type="text/javascript" > 
  ...
  function deleteOrg(orgId, orgName){
  		jQuery('#dialogDeleteOrg p').html("${deleteOrgConfirm}" + "<span class='new-manage-org-name-alert'>" + orgName + "</span>&nbsp;" +"<%= res.getString("message_question")%>");
	         ...
	         buttons: {
	           	"<%=res.getString("yes")%>" : function() {
	 				 ...
	 				 jQuery.post(delete_url, {"orgId": orgId, "action":"delete"}, function(){
	 				   ...
	 				   jQuery('#dialogDeleteOrg p').html("${deleteOrgMessage}" + "<span class='new-manage-org-name-alert'>" + orgName + "</span>&nbsp;" +"<%= res.getString("message_success")%>");
					   jQuery('#dialogDeleteOrg').dialog({
					    	 modal:true,
					       	 resizable: false,
					       	 height: 128,
  ...
  function deleteManyOrg(){
		...
		if (val.join(";") == ''){
			...
			jQuery('#dialogDeleteOrg').dialog({
		        ...
		        jQuery('#dialogDeleteOrg p').html("<%=res.getString("message_delete_confirm")%>");
		    	jQuery('#dialogDeleteOrg').dialog({
			       	 modal:true,
			       	 resizable: false,
			       	 height: 115,
