								...
								<span class="pg-normal-content-news-org"> <!-- sua chua-->
									<c:choose>
										<c:when test="${isOrgAdmin eq true}">
											...
											<a
												href="${repairNewsPath}&articleId=${article.journalArticleId}">
												<%=res.getString("repair")%></a>
										</c:when>
									</c:choose> <a class="pg-content-link-news-org"
									href="${hostAdd}&articleId=${article.journalArticleId}">
										${article.title} </a> <br /> <!--
												<c:choose>
												    <c:when test="${isOrgAdmin eq false}">
														<a style="color: #30A1BE;float:right;" href="${hostAdd}&articleId=${article.journalArticleId}"> Xem Thêm >> </a>
													</c:when>
											    </c:choose>
											 --> </span>
							</div>
