...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {
			...
			// Add OT REQ
			jQuery('input.send').click( function(e) {
				...
				if(chk)
				{
					...
					var regDate= jQuery('.date-req').val();
					...
					jQuery.ajax({
						...
						error: function(data) {
							jQuery("#backgroundLoading").hide();
							alert("Đã có lỗi xảy ra, vui lòng thông báo đến quản trị viên ProGate");
							jQuery(this).removeAttr('disabled');
...
<form action="" name="frmRequestOTDetail" method="post">
	...
	<div class="cls-ot">
			...
			<table id="tbl-field">
				...
				<tbody>
					<tr>
						...
						<td>
							<input class="date-req" type="text" name="today" value="<fmt:formatDate pattern="dd/MM/yyyy" value="${bean.now}" />" readonly="readonly" />
						</td>
					</tr>
					<tr>
						<td class="cls-org">
						...
						<td><a></a>
							<select name="slt-org" class="slt" onchange="myOrg();">
								<c:forEach items="${bean.lsOrg }" var="item" varStatus="i">
									<c:choose>
										<c:when test="${item.orgId==bean.orgIdChange }">
											<option value="${item.orgId}" selected="selected">${item.orgName }</option>
										</c:when>
										<c:otherwise>
		...
		<div class="content-right">
			...
			<div class="date-ot">
				<fmt:message key="lmis.ot.reg.regday" /> <input style =" width:74%;" value="<fmt:formatDate pattern="dd/MM/yyyy" value="${bean.now}" />" class="date-input-ot" type="text" name="date-ot" />
			</div>
		...
		<div class="clear"></div>
		<div class="tbl-mbr">
			<table id="tbl" border="1" bordercolor="#C0C0C0" >
				<thead>
					<tr>
						...
						<th scope="row" width="10%" style="border: 1px solid #c0c0c0" title="Giờ bắt đầu"><fmt:message key="lmis.attendanceInfo.startTime" /></th>
				...
				<tbody >
					<c:forEach var="item" items="${bean.lsStaff }" varStatus="i">
						<c:choose>
	...
	<div class="button">
		...
		&nbsp
		<input class="send" type="button" value="<fmt:message key="button.sendform" />"  />
	</div>
</form>
