...
<%-- 				<c:choose> --%>
<%-- 					<c:when test="${bean.selectedOrgId==-1}"> --%>
<%-- 						<option selected="selected" value="-1"><fmt:message key="lmis.common.all"/></option> --%>
<%-- 					</c:when> --%>
<%-- 					<c:otherwise> --%>
...
<%-- 				</c:choose> --%>
				<c:forEach var="subOrg" items="${bean.listSubOrg}" varStatus="countItems">
					<c:choose>
						<c:when test="${bean.selectedOrgId == subOrg.orgId}">
			...
			<div class="selected-status">
				...
				<select name="selectedStatus" style="width: 150px;">
						<option id="all" value="-1" selected><fmt:message key="lmis.common.all"/></option>
			...
			<div class="search-button">
				<img  align="absmiddle" alt="" src="/ProGate-portlet/images/lmis/Search.png"><span style="color:orange;"><fmt:message key="button.view"/></span>
			</div>
		</div>
	<c:choose>
