			...
			<table class="table" style="overflow-y:auto;overflow-x:hidden;width:340%" >
				<thead style="color:#ffffff">
					<tr>
	    				...
	    				<th width="90px" rowspan="2">Mã NV</th>
	    				<th width="300px" rowspan="2">Họ Tên Nhân Viên</th>
	    				<th width="84px"rowspan="2">TS Ngày Đã Nghỉ</th>
    					...
    					<c:choose>
							...
							<c:otherwise>
								<tr class="evenrow">
									...
									<td class="cls-stt-right">${functions:getTotalDayOff(items.userId,bean.month,bean.year,bean.orgId)}</td>
									<c:forEach var="item" items="${bean.lsNameAb}">
										<td class="cls-stt-right">${functions:getAbsenceDetail(items.userId,bean.month,bean.year,bean.orgId,item.lmisAbsenceSettingsId,1)}</td>
										<td class="cls-stt-right">${functions:getAbsenceDetail(items.userId,bean.month,bean.year,bean.orgId,item.lmisAbsenceSettingsId,2)}</td>
										<td class="cls-stt-right">${functions:getAbsenceDetail(items.userId,bean.month,bean.year,bean.orgId,item.lmisAbsenceSettingsId,3)}</td>
									</c:forEach> 
								</tr>
						</c:forEach>
				</tbody>
