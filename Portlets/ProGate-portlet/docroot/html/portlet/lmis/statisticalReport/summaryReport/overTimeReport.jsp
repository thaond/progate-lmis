...
<form action="" name="frmOverTimeReport" id="frmOverTimeReport" method="post">
	<div class="content">
		...
		<div class="tableContainer" style="overflow:auto">
			<table class="table" style="overflow-y:auto;overflow-x:hidden;width:150%" >
				<thead style="color:#ffffff">
					<tr>
						<th class="styleSTT" style="height:23px" rowspan="2"><fmt:message key="lmis.common.number"/></th>
	    				<th width="60px">Mã NV</th>
	    				<th width="170px">Họ Tên Nhân Viên</th>
    					...
    					<c:choose>
							...
							<c:otherwise>
								<tr class="evenrow">
									...
									<td class="cls-num-right">${functions:getTotalDayAbsenceCompensasion(items.userId,bean.month,bean.year,bean.orgId,1)}</td>
									<td class="cls-num-right">${functions:getTotalDayAbsenceCompensasion(items.userId,bean.month,bean.year,bean.orgId,2)}</td>
									<td class="cls-num-right">${functions:getTotalDayAbsenceCompensasion(items.userId,bean.month,bean.year,bean.orgId,3)}</td>
									<td class="cls-num-right">${functions:getDayCompensationNextMonth(items.userId,bean.month,bean.year,bean.orgId)}</td>
									<c:forEach var="item" items="${bean.lsName}" varStatus="itemCount">
										<td class="cls-num-right">${functions:getTotalHourOT(items.userId,bean.month,bean.year,bean.orgId,item.lmisOverTimeTypesId)}</td>
									</c:forEach>
									<td class="cls-num-right">0.0</td>
									<td class="cls-num-right">${functions:getOverTimeHour(items.userId,bean.month,bean.year,bean.orgId)}</td>			
									<td class="cls-num-right">${functions:getTotalHourOverTimeReward(items.userId,bean.month,bean.year,bean.orgId)}</td>
							</tr>
					</c:forEach>
				</tbody>
			</table>
