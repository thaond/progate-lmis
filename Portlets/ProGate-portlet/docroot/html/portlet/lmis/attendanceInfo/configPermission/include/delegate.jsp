...
<jsp:include page="../../../dialog/dialog-delegateOk.jsp" />
<%@ page contentType="text/html;charset=UTF-8" pageEncoding="UTF-8" %>

<!-- <form name="delegate" type="post"> -->
			...
			<table class="table-content">
				<thead class="tbl-header" style="display:block; width: 99.9%; margin-left: -1px;">
					<tr class="header_table" >
						...
						<th scope="col" height="29px" width="300px" class="align-center border-style"><fmt:message key="lmis.config.code.staffname" /></th>
				...
				<c:forEach items="${bean.listUserId}" var="uId" varStatus="count">
				 <c:choose>
				 	...
				 	<c:otherwise>
						...
						<td width="300px" style="text-indent: 10px"> ${functions:getUserNameByUserId(uId.userId)}<br /></td>
...
<!--						<th width="20px" scope="col" height='30px'><input id="checkAll" name="checkAll" type="checkbox" /> </th>-->
						<th width="420px" scope="col" height='30px'><fmt:message key="lmis.configPermission.listOfFunction" /></th>
						
					</tr>
				</thead>
				<tbody class="display_block" id="listPermit">
				  <c:forEach items="${bean.userPer}" var="userPer" varStatus="countStatus">
				  <c:set var="name" value="" scope="page"></c:set>
				  <c:if test="${userPer.proGatePermissionsRolesId == bean.delegatePer}">
				  	<c:set var="name" value="disabled" scope="page"></c:set>
				  </c:if>
					<c:choose>
						...
						<td style="width:20px; text-align: center">
							<input id="${userPer.proGatePermissionsRolesId}" style="margin-left:8px" type="checkbox" name="checkList" <c:out value='${pageScope.name}'/> value="${userPer.proGatePermissionsRolesId}"/>
						</td>
						<td width="400px" style="padding-left:10px">
							<label for="${userPer.proGatePermissionsRolesId}">${userPer.description}</label><br />
						</td>
				</tr>
			  </c:forEach>
			</tbody>
...
jQuery(document).ready(function() {
	...
	<c:forEach items="${bean.grantedPer}" var="up">
...
jQuery('#savePermission').click(function(e){
	...
	var href = '<portlet:renderURL  windowState="<%=LiferayWindowState.EXCLUSIVE.toString() %>">
		<portlet:param name="action" value="commonController" />
		<portlet:param name="<%=Constants.CMD%>" value="<%=LmisRequest.GRANT%>" />
		<portlet:param name="<%=Request.ORG_ID %>" value="${bean.orgId}" />
	</portlet:renderURL>';
	jQuery.ajax({
		type:'POST',
		...
		error: function(){
			alert("Đã có lỗi xảy ra khi lưu ủy quyền!");
		}
	});
});
