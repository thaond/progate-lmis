package com.larion.progate.news_homepage.controllers;
...
public class UpdateNewsController extends
...
//				.getAttribute(WebKeys.THEME_DISPLAY);
		...
		articleId = Integer.parseInt(uploadPortlet.getParameter(News_Const.ARTICLE_ID));
		...
		try {	
		// Get current User
			com.liferay.portal.service.ServiceContext sc = ServiceContextFactory.getInstance(this.getClass().getName(), request);
			...
			if (articleId != 0){
				...
				if (mark != null){
					article.setPriority(ProGateJournalArticlePrioritiesLocalServiceUtil.getPriorityImportantId());
				}else{
					article.setPriority(ProGateJournalArticlePrioritiesLocalServiceUtil.getPriorityNormalId());
				}
				
				article.setUpdatedBy(userId);
				article.setUpdatedAt(new Date());
				
				String fileName = uploadPortlet.getFileName(News_Const.SMALL_IMAGE);
				if (fileName != ""){
					String smallImagePath = Functions.getPathOfUploadImage(request, response, uploadPortlet, News_Const.SMALL_IMAGE);					
				...
				addSuccess = 1;
				msg = "Bạn đã cập nhật tin tức thành công."; 
		...
		addSuccess = 2;
		msg = "Đã có lỗi xảy ra trong quá trình cập nhật tin tức."; 
	...
	protected ModelAndView handleRenderRequestInternal(RenderRequest request,
		...
		int isAdd = 0;
		int addSuccess = 0;
		int articleId = Integer.parseInt(request.getParameter(News_Const.ARTICLE_ID));
		...
		if(request.getParameter(News_Const.ADD_SUCCESS) != null){
			addSuccess = Integer.parseInt(request.getParameter(News_Const.ADD_SUCCESS));
		}
		String msgText = "";
