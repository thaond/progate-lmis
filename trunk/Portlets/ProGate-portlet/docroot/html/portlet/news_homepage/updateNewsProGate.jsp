				...
				<div class="video-body-mes-success news-msg" id="mesSuccess">
					...
					<div style="float: left; padding-top: 8px; padding-left: 5px;">
						<span class="tq-body-content-mes"> 
							<c:choose>
								<c:when test="${bean.cmd == 1 }">
									<fmt:message key="news.sucAdd"/>
								</c:when>
								<c:otherwise>
									<fmt:message key="news.sucEdit"/>
								</c:otherwise>
							</c:choose>
						</span>
					</div>
				...
				<div class="video-body-mes-error news-msg" id="mesError">
					...
					<div style="float: left; padding-top: 8px; padding-left: 5px;">
						<span class="tq-body-content-mes">
							<c:choose>
								<c:when test="${bean.cmd == 1 }">
									<fmt:message key="news.errAdd"/>
								</c:when>
								<c:otherwise>
									<fmt:message key="news.errEdit"/>
								</c:otherwise>
							</c:choose>
						</span>
					</div>
									 ...
									 <tr>
								    	...
								    	<td>
								    		<div class="news-org-img-border" align="center">
								    			<img alt="" src="${article.smallImageUrl}" id="smallImagePreview" onLoad='scaleImg(this, 145, 100)' style="display: block;">
								    		</div>
...
<script type="text/javascript">
	...
	jQuery('#updateNewsProGate input[type=file]').change(function(el) {
		// Image preview
		...
		if (confirm('<fmt:message key="warning.confirm.delete"/>')) {
			var portletActionURL = 
				'<portlet:actionURL windowState="<%=WindowState.NORMAL.toString() %>">
					<portlet:param name="action" value="deleteNewsProGate" />
