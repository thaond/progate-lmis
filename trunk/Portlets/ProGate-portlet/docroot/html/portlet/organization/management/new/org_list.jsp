...
<%@page import="larion.progate.service.ProgateOrganizationParticipantsLocalServiceUtil"%>
...
<div class="right-content" id="<portlet:namespace/>library">
	...
	<c:choose>
		<c:when test="${(userType < 0 || userType > 4) && isProGateAdmin == false}">			
	...
	<div style="width:100%;margin-top: 12px;">
		<span class="new-manage-org-header">
			<c:choose>
				<c:when test="${isProGateAdmin == true}"> <%=res.getString("org.other")%> </c:when>
				<c:when test="${filterType == 0}"> <%=res.getString("org.admin")%> </c:when>
				<c:when test="${filterType == 1}"> <%=res.getString("org.member")%> </c:when>		
				<c:when test="${filterType == 3}"> <%=res.getString("org.wait")%> </c:when>
				<c:when test="${filterType == 4}"> <%=res.getString("org.invite")%> </c:when>				
				<c:otherwise> <%=res.getString("org.other")%> </c:otherwise>
			</c:choose>	 
	 	</span>	
	 	
	 	<c:if test="${countList > 0 || (countList == 0 && keyWords!= null)}">		 	
		 	<input type="text" name="search-key" id="search-key" class="new-manage-org-search" maxlength="50" class="text" value="${keyWords}"/>	
		 	<script type="text/javascript">
		 	...
		 	<button id="timkiem" class="bt-form" style="height:21px;width:80px;">	
		...
		%>		
			...
			<c:if test="${isProGateAdmin == true}">
				<div align="right" class="new-manage-org-action-head">
					...
					&nbsp;|&nbsp;
					<a class="new-manage-org-link" href="javascript:void(0);" onClick="deleteManyOrg()"><%=res.getString("delete.org")%></a>
					&nbsp;|&nbsp;
					<a class="new-manage-org-link" href="javascript:void(0);"><%=res.getString("configuration.org")%></a>
				</div>
			</c:if>
			...
			<c:forEach var="org" items="<%= listOrgs %>" varStatus="rowCounter">
				...
				<c:choose>
					<c:when test="${!(org.orgStatus == false && (filterType == 1 || filterType == 3 || filterType == 4))}">
							...
							<div class="new-manage-org-logo-org">						
								<a href="${directPath}?<%=URLParameter_RS.ORG_ID%>=${orgId}&<%=URLParameter_RS.VIEW_MODE%>=<%=RSConstants.VIEW_MODE%>">
									...
									<c:choose>								
										...
										<c:otherwise>
											<div class="logo" align="center"><img id="anh${rowCounter.count}" alt="" ></div>
							...
							<div class="new-manage-org-content">
								...
								<c:choose>
									<c:when test="${org.isProactive == true}">
										<img src="<%=request.getContextPath()%>/images/new-manage-org/PremierPro.png" class="hightlights-org-enable" id="typical-normal-image-${org.orgId}"></span>
									</c:when>	
									<c:otherwise>
										<img src="<%=request.getContextPath()%>/images/new-manage-org/PremierPro.png" class="hightlights-org-disable" id="typical-normal-image-${org.orgId}"></span>
									</c:otherwise>									
								</c:choose>
								<br />
