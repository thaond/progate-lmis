...
<div class="menu-org-container">
	<div class="menu-org-header">
		<% if(isRepresentOrgPage){ %>
		<span style="font-size: 16px;" class="menu-org-tuade">Tổ Chức
			Tiêu Biểu</span>
		<% }else{ %>
		...
		<c:if test="${countList > 0 || (countList == 0 && keyWords!= null)}">
			<input type="text" name="search-key" id="search-key"
				...
				value="${keyWords}" style="width: 450px;" />
			<script type="text/javascript">
	...
	<%	} else {
	%>
	<div id="org-list-content" class="menu-org-content"
		style="float: left;">
					...
					<div class="menu-org-folder-news">
						<div class="menu-org-logo-org">
							<a
								...
								<c:set value="${fn:trim(org.logoId)}" var="logo" /> <c:choose>
									...
									<c:otherwise>
										<div class="logo">
											<img id="anh${rowCounter.count}" src="${logo}" alt=""
												class="logoImg" onLoad='scaleImg(this, 150, 120)'
												style="display: none;">
										</div>
									</c:otherwise>
								</c:choose> </a>
						...
						<div class="menu-org-content">
							<span align="left" class="new-manage-org-name"> <a
								...
								</c:if> </span> <br /> <span style="color: gray;">1/${org.maxUserCount}
								<%=res.getString("member")%>&nbsp;|&nbsp; ${org.city}
								&nbsp;|&nbsp; ${org.orgType} &nbsp;|&nbsp; <c:choose>
...
<script type="text/javascript">
	...
	&currentPage="
					+ currentPageNum + "&pageSize=" + currentPageSize;
<%}%>
	}
	}
</script>
