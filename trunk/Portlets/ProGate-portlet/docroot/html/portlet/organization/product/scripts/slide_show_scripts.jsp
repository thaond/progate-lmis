...
<script type="text/javascript">
	...
	var is_no_choosen = true;
	
	function submit(actionUrl) {
		if(is_no_choosen == true){
			...
			jQuery("#tnt-global-message span").text('<fmt:message key="warning.slideshow.no.choosen" />');
		}
		else if ( valid_slideshow_product_imageurl_1 == true &&
	...
	function addSubmit() {
		...
		var portletActionURL = 
			'<portlet:actionURL windowState="<%=WindowState.MAXIMIZED.toString()%>">
				<portlet:param name="action" value="handleSlideShowProduct"></portlet:param>
	...
	jQuery(function(){
		...
		jQuery(".file-target").change(function() {
			...
			if (this.files) {
			  ...
			  jQuery(this).parent().parent().parent().parent().parent().find(".image-preview").attr("src", this.files[0].getAsDataURL());
			 ...
			 // jQuery("#image-preview").attr("src", jQuery("#file-target").val());
			  jQuery(this).parent().parent().parent().parent().parent().find(".image-preview").attr("src", jQuery(this).val());
