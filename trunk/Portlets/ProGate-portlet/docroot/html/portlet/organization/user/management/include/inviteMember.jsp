...
<script type="text/javascript">
	...
	jQuery(function() {
		jQuery(".tnt-bt-invite").click(function(){
...
<div class="tnt-row">
	<div class="tnt-control">
		...
		<div class="tnt-row-right">
			<div class="tnt-control-action">				
				<a class="tnt-bt-invite" href="javascript:void()">
					<fmt:message key="title.user.invite"/>
				</a>
			</div>
		</div>
	</div>
	...
	<tbody>
		...
		<c:if test='${bean.searchPhrase != ""}'>
		<c:choose>
			<c:when test="${bean.countListUsers == 0}">
			<tr>
			<td colspan="5" class="tnt-warning-message">
			...
			<c:otherwise>
			<c:forEach items="${bean.userList}" var="user" varStatus="count">
			<c:choose>
	...
	<c:if test='${bean.searchPhrase != "" && bean.countListUsers > 0}'>
		<div class="tnt-row">
