...
<%
	...
	if(request.getAttribute(News_Const.IS_EDITABLE) != null){
		isEditable = (Boolean)(request.getAttribute(News_Const.IS_EDITABLE));
	}
...
<form name="pagingForm" method="post" action="">
	<div class="container">
	      <div style="height:30px;margin-top: 10px;margin-left: 10px">
	      	<span class="tuade" style="font-size: 16px;"><fmt:message key="blog.header"/></span>
					  	 ...
					  	 <div class="folder-news-pg"> 
					  	 	  <div class="news-org-img-border" align="center">       
			               	  	<a href="<%= detailNewsUrl%>${article.primaryKey}">
				          			...
				          			<c:choose>
				          				<c:when test="${smallImageUrl == ''}">
			                    			<img class="img-subject fl" alt="" src="<%=request.getContextPath()%>/images/news_org/small_noImage.gif">
			                 	 		</c:when>
					          		    <c:otherwise>
					          		    	<img alt="" src="${article.smallImageUrl}" onLoad='scaleImg(this, 145, 100)' style="display: none;">
			                 	 		</c:otherwise>
