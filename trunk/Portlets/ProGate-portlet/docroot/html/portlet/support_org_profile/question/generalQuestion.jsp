...
<div class="support-org">    
  <div class="container">
	<!-- Câu Hỏi Thường Gặp -->
 	<div id="question" class="support-distance">
      	...
      	<span class="header"><%= res.getString("question")%></span>
      	<span class="add-support-org">
      		<c:if test="${isOrgAdmin eq true}">
		    	<a class="pg-content-link" style="color:#30A1BE;" href='${addQuestionPath}' >
		          <%=res.getString("add")%>
		        </a>
	        </c:if>
	    </span>
	 ...
	 <div class="list-content">
	 	 <div class="folder-news">
	 	 	<img src="<%=request.getContextPath()%>/images/support_org_profile/information.png" style="padding-right:5px;vertical-align: middle;float:left;" width="20" height="20"/>
	 	 	<div class="text-guilde">
	 	 		<%= res.getString("guideQuestion")%>
	 	 	</div>
	 	 </div>
	 	 <div id="sortable">
	 	 	<c:if test="${count > 0}">
	 	 		<c:forEach var="article" items="${listArticles}" varStatus="rowCounter">
			       <div class="folder-news-question">
			       	   <img src="<%=request.getContextPath()%>/images/support_org_profile/question.png" style=" padding-top: 10px;padding-right:5px;vertical-align: middle;float:left;" width="18" height="18"/>
			           <div class="small-image-other" style="padding:10px;">
		            		<c:if test="${isOrgAdmin eq true}">
				            	<img src="<%=request.getContextPath()%>/images/news_org/write.png" style="vertical-align:bottom;" width="15" height="15" />
				             	<a href="${repairQuestion}&articleId=${article.journalArticleId}" ><%= res.getString("repair")%></a>
				          	</c:if>
		            		<a href="${detailURL}&actionView=detailQuestion&articleId=${article.journalArticleId}" class="question-link">
			           			${article.description}
			           		</a>
			            </div>
			        </div>
...
<script>
	...
	jQuery("#question").attr("style","height:30px;margin-top:10px;");
