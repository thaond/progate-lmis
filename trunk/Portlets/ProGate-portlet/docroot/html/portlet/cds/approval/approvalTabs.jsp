...
<ul class="tabs ">
	...
	<c:choose>
		<c:when test="${bean.activedTab == bean.TAB_APPROVAL_1}">
			<li><a href='${tab1}' class='actived'>Phê duyệt từng nhân viên</a></li>	
		</c:when>
		<c:otherwise>
			<li><a href='${tab1}'>Phê duyệt từng nhân viên</a></li>
		</c:otherwise>
	</c:choose>
	</c:if>
	<c:if test="${bean.hasPermissionOnTab1 == false}">
		<li><a style="background-color: rgb(220, 219, 218); color: gray; cursor:default; border: 1px solid lightgray; border-bottom: none;">Phê duyệt từng nhân viên</a></li>
	</c:if>
	
	<c:if test="${bean.hasPermissionOnTab2 == true}">
	<c:choose>
		<c:when test="${bean.activedTab == bean.TAB_APPROVAL_2}">
			<li><a href='${tab2}' class='actived'>Phê duyệt nhanh</a></li>	
		</c:when>
		<c:otherwise>
			<li><a href='${tab2}'>Phê duyệt nhanh</a></li>
		</c:otherwise>
	</c:choose>
	</c:if>
	<c:if test="${bean.hasPermissionOnTab2 == false}">
		<li><a style="background-color: rgb(220, 219, 218); color: gray; cursor:default; border: 1px solid lightgray; border-bottom: none;">Phê duyệt nhanh</a></li>
	</c:if>
