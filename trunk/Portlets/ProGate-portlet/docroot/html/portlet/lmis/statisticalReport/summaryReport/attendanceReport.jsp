...
<form action="" name="frmAttendanceReport" method="post">
	<div class="content">
    	...
    	<div class="tableContainer" style="overflow:auto">
    		<table class="table" style="overflow-y:auto;overflow-x:hidden;width:170%" >
    			<thead style="color:#ffffff">
    				<tr>
    				<th class="styleSTT"><fmt:message key="lmis.common.number"/></th>
    				<th width="90px">Mã NV</th>
    				<th width="300px">Họ Tên Nhân Viên</th>
    				<th width="84px" title="Giờ bắt đầu làm việc trung bình trong tháng">GBDLVTB Trong Tháng</th>
    				<th width="85px" title="Giờ bắt đầu làm việc trung bình trong năm">GBDLVTB Trong Năm</th>
    				<th width="84px" title="Giờ kết thúc làm việc trung bình trong tháng">GKTLVTB Trong Tháng</th>
    				<th width="85px" title="Giờ kết thúc làm việc trung bình trong năm">GKTLVTB Trong Năm</th>
    				<th width="96px" title="Thời gian làm việc">Thời Gian LV (Giờ)</th>
    				<th width="100px" title="Thời gian đi trễ">TGDT (Phút)</th>
    				<th width="99px"  title="Thời gian đi trễ bị phát">TGDT BP (Phút)</th>
    				<th width="104px" title="Tổng thời gian đi trễ bị phạt đã nhân hệ số">TTGDT BP (Phút) Đã Nhân Hệ Số</th>
    				<th width="100px" title="Thời gian về sớm">TGVS (Phút)</th>
    				<th width="100px" title="Thời gian về sớm bị phạt">TGVS BP (Phút)</th>
    				<th width="110px" title="Tổng thời gian về sớm đã nhân hệ số">TTGVS BP (Phút) Đã Nhân Hệ Số</th>
    				</tr>
    			</thead>
    			<tbody>
    					...
    					<c:choose>
							...
							<c:otherwise>
								<tr class="evenrow">
									...
									<td class="cls-stt-right">${functions:getTotalWorkingTime(items.userId,bean.month,bean.year,bean.orgId)}</td>
									<td class="cls-stt-right">${functions:getTimeLate(items.userId,bean.month,bean.year,bean.orgId)}</td>
									<td class="cls-stt-right"> ${functions:getTimeLatePernalty(items.userId,bean.month,bean.year,bean.orgId)}</td>
									<td class="cls-stt-right"> ${functions:getTimeLateFinal(items.userId,bean.month,bean.year,bean.orgId)}</td>
									<td class="cls-stt-right"> ${functions:getTimeEarly(items.userId,bean.month,bean.year,bean.orgId)}</td>
									<td class="cls-stt-right"> ${functions:getTimeEarlyPernalty(items.userId,bean.month,bean.year,bean.orgId)}</td>
									<td class="cls-stt-right" > ${functions:getTimeEarlyFinal(items.userId,bean.month,bean.year,bean.orgId)}</td>
								</tr>
					</c:forEach>
    		</table>
    	</div><!-- tableContainer -->
