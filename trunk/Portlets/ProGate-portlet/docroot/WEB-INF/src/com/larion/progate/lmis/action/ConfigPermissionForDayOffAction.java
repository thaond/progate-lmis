...
public class ConfigPermissionForDayOffAction extends ParameterizableViewAction {
	...
	public void handleRenderRequestInternal(RenderRequest request,
		...
		int categoryId = LmisConst.Pers_Absence ; // attendance info module
		...
		String updateDayOffPer = LmisConst.Update_Permision_DayOff;
		String authorizedDayOffPer = LmisConst.Authorized_DayOff;
		int updateDayOffPermission = ViewOrgUsersPermissionsLocalServiceUtil.checkUserSpecialPermission(appName, category, rootId, IdOfUser,  updateDayOffPer);
		int authorizedDayOffPermission = ViewOrgUsersPermissionsLocalServiceUtil.checkUserSpecialPermission(appName, category, rootId, IdOfUser,  authorizedDayOffPer);
		
		bean.put("updatePermission", updateDayOffPermission);
		bean.put("authorizedPermission", authorizedDayOffPermission);
		String defaultTab = LmisRequest.TAB_BUILDING_PERMIT_1;
		if(updateDayOffPermission==0 && authorizedDayOffPermission !=0){
	...
	//				System.out.println(userPer.toString());
	//				System.out.println("userPermission ````````````````````````````````");
		...
		bean.put("delegatePer", LmisConst.Authorized_DayOff);
	}
