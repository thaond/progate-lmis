	...
	public void handleRenderRequestInternal(RenderRequest request,
		...
		try {
			...
			if (productId != 0) { // Showing product details
				...
				//check curproduct is the last of relateProductList
				boolean isLastProduct = false;
				if(relateProductList.get(relateProductList.size()-1).equals(curProduct)){
					isLastProduct = true;
				}

				bean.put("isLastProduct", isLastProduct);
